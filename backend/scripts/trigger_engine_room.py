...
def run():
    logger.info("Triggering Engine Room Workflow...")
    ...
    logger.info("Running pipeline (until human review)...")
    # This will run until the human_review node interrupts it
    for event in graph.stream(state, config=config):
        for key, value in event.items():
            logger.info(f"Node '{key}': {value}")
            
    # Check if we hit the interrupt
    current_state = graph.get_state(config)
    if not current_state.next:
        logger.info("Pipeline finished (no insights to review).")
        return
        
    logger.info(f"\n[!] PIPELINE PAUSED AT: {current_state.next}")
    logger.info("Pending values:")
    ...
    if ans.lower() == 'y':
        logger.info("Approving update...")
        # Inject the approval into the state
        graph.update_state(config, {"approved": True})
    else:
        logger.info("Rejecting update...")
