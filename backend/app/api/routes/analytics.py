...
async def process_pipeline(request: Request, payload: PipelineRequest, background_tasks: BackgroundTasks, user_id: str = Depends(get_current_user_id)):
    ...
    # Start a parent span for the entire request
    with tracer.start_as_current_span("api_process_pipeline") as span:
        span.set_attribute("ticker", payload.ticker)
        span.set_attribute("timeframe", payload.timeframe)
