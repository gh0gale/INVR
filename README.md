...
<img src="./outputs/workspace.png" alt="INVR Workspace Overview" width="920" />

### AI Tutor Terminal

<img src="./outputs/terminal.png" alt="INVR Tutor Terminal" width="920" />

### Architecture

<img src="./outputs/arch.png" alt="INVR Architecture overview" width="920" />
...
└────────┬────────┘
         │ (REST / SSE Streams)
         └──────────────► FastAPI Backend
                          ├── Bronze Layer (Data Fetching via YFinance, NSEPython)
                          ├── Silver Layer (Vectorized Math: Pandas, NumPy)
                          ├── Gold Layer (Algorithmic Gates & Trade Setups)
                          └── LLM Orchestrator (LangGraph, LangChain, Ollama)
                     ...
                     Engine Room (Cron/Scheduled Python Scripts)
                     ├── grade_ledger.py
                     ├── analyze_drift.py
                     └── simulate_live_history.py
```
